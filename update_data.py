...
Gira ogni lunedì via GitHub Actions, tutto l'anno, silenziosamente.

IN STAGIONE:    aggiorna risultati, orari, classifica
...
def parse_results(html):
    ...
    for m in pat.finditer(plain):
        ...
        dd,mm,yyyy = dr.split("/")
        results.append({"date":f"{yyyy}-{mm}-{dd}","time":t,"home":h.strip(),"away":a.strip(),"sh":int(sh),"sa":int(sa)})
    return results
...
def find_match(scraped, match):
    ...
    for s in scraped:
        ...
        if abs((sd-md).days) > 4: continue
        if sh in mh or mh in sh or ("virtus roma" in mh and "virtus" in sh) or ("luiss" in mh and "luiss" in sh):
            return s
    return None
...
def update_in_season(matches):
    today = date.today()
    rounds_done = [m["round"] for m in matches if m.get("sh") is not None and m.get("phase")=="regular"]
    ...
    for rnd in range(max(1, last_round), 39):
        if rnd in found_rounds: continue
        base_id = 356237 + (rnd - 31) * 1500
        urls = []
        for delta in range(-500, 501, 100):
            cid = base_id + delta
            urls += [
        ...
        for url in urls:
            ...
            seen.add(url)
            if rnd in found_rounds: break
            html = fetch(url)
            if not html or len(html) < 1000: continue
            if "girone b" not in html.lower(): continue
            scraped = parse_results(html)
            if scraped:
                print(f"  ✅ Giornata {rnd}: {len(scraped)} risultati")
    ...
    for m in matches:
        ...
        if md < today and m.get("sh") is None:
            ...
            if found:
                ...
                if found.get("time"): m["time"] = found["time"]
                print(f"  ✅ {m['home']} vs {m['away']}: {found['sh']}-{found['sa']}")
                updated += 1
        if md >= today:
            found = find_match(all_scraped, m)
            if found and found.get("time") and found["time"] != m.get("time"):
                m["time"] = found["time"]; updated += 1
    return updated

# ================================================================
# FUORI STAGIONE — cerca nuovo calendario
# ================================================================
...
def search_new_calendar(next_season):
    ...
    for url in pdf_urls:
        ...
        if html and len(html) > 500 and ("virtus" in html.lower() or "luiss" in html.lower()):
            ...
            return parse_new_calendar_pdf(html, slug)

    # Prova pagina calendario LNP web
    lnp_urls = [
    ...
    for url in lnp_urls:
        ...
        if html and slug in html:
            print(f"  🌐 Nuova stagione trovata su LNP: {url}")
            return None, slug  # segnala che c'è qualcosa ma non parsare ancora

    # Cerca su pianetabasket
    pb_html = fetch(f"https://www.pianetabasket.com/serie-b/?s=giornata+{slug}")
    if pb_html and ("virtus" in pb_html.lower() or "luiss" in pb_html.lower()):
        links = re.findall(r'href="(/serie-b/[^"]*' + re.escape(slug) + r'[^"]*)"', pb_html)
        for link in links[:3]:
            page = fetch("https://www.pianetabasket.com" + link)
            if page and len(page) > 1000:
                print(f"  📰 Trovata pagina {slug} su pianetabasket")
                return None, slug

    print(f"  ℹ️  Nessun calendario {next_season} ancora disponibile")
    return None, None


def parse_new_calendar_pdf(text, season_slug):
    """Estrae partite in casa dal testo PDF LNP."""
    new_matches = []
    lines = text.split("\n")
    for line in lines:
        line = line.strip()
        ...
        hn = normalise(home)
        team = None
        if "virtus roma" in hn or "virtus gvm" in hn: team = "virtus"
        elif "luiss" in hn: team = "luiss"
    ...
    return new_matches, season_slug
...
def main():
    ...
    if data_path.exists():
        ...
        matches, standings, config = [dict(m) for m in BASE_MATCHES], dict(BASE_STANDINGS), CONFIG
    ...
    if in_season:
        ...
        print(f"📝 Aggiornamenti applicati: {total_updated}")
    else:
        print(f"\n💤 Modalità: FUORI STAGIONE — cerco calendario {next_season}")
        result = search_new_calendar(next_season)
        new_matches, found_season = result if isinstance(result, tuple) else (result, None)

        if new_matches and found_season:
            print(f"🆕 Nuovo calendario {found_season} trovato — aggiorno")
            matches   = new_matches
            standings = {"virtus":{"pos":0,"pts":0,"w":0,"l":0},"luiss":{"pos":0,"pts":0,"w":0,"l":0}}
            yr = int(found_season[:4])
            config["season"]      = found_season
            config["next_season"] = f"{yr+1}-{str(yr+2)[2:]}"
            total_updated = len(new_matches)
        elif found_season:
            print(f"ℹ️  Stagione {found_season} rilevata — calendario non ancora disponibile")
        else:
            print("ℹ️  Nessuna novità — nessuna modifica")
