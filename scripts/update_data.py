...
KNOWN_URLS = {
    ...
    # 33-38: trovati automaticamente dalla homepage
}

# ID base stimati per le giornate future
# G32 = 357140, incremento medio ~1500 per giornata
ROUND_BASE_IDS = {
...
def get_urls_for_round_fast(rnd):
    """
    Genera al massimo 3 URL da provare per una giornata:
    1. URL noto verificato (se disponibile)
    2. ID base + suffisso principale
    3. ID base-200 + suffisso domenica
    """
    urls = []
    ...
    # Solo i 2 suffissi più probabili per evitare troppe richieste
    for suf, delta in [("classifiche", 0), ("sabato-classifiche", 0),
                       ("domenica-classifiche", -200)]:
        url = (
...
def fetch_schedule_sofascore():
    """
    Cerca orari partite future su Sofascore (API JSON non ufficiale).
    Timeout breve per non rallentare se non risponde.
    """
    schedules = []
    # Tournament ID Serie B Nazionale basket su Sofascore = 14251 (verificato)
    for tid in [14251, 5968, 5969]:
        ...
        try:
            ...
            for s in data.get("seasons", []):
                year = str(s.get("year", ""))
                if "2025" not in year and "2026" not in year:
                    continue
                sid = s.get("id")
                if not sid:
                    continue
                ev_url = (
                    f"https://api.sofascore.com/api/v1/unique-tournament/{tid}"
                    f"/season/{sid}/events/next/0"
                )
                ev_html = fetch(ev_url, timeout=4)
                if not ev_html:
                    continue
                ev_data = json.loads(ev_html)
...
def update_in_season(matches, config, standings):
    ...
    # STEP 3: Orari futuri da Sofascore e siti squadre
    ...
    for m in matches:
        ...
        # Orario partita futura — da pianetabasket
        if md >= today:
            found = find_match(all_scraped, m)
            if found and found.get("time") and found["time"] != m.get("time"):
                print(f"  🕐 {m['home']} vs {m['away']}: {found['time']} (pianetabasket)")
                m["time"] = found["time"]
                updated += 1

        # Orario partita futura — da Sofascore/siti squadre
        if md >= today:
            for sched in extra:
                try:
                    smd = datetime.strptime(sched["date"], "%Y-%m-%d").date()
                    if (sched.get("team") == m["team"] and
                            abs((smd - md).days) <= 2 and
                            sched.get("time") and sched["time"] != m.get("time")):
                        print(f"  🕐 {m['home']} vs {m['away']}: {sched['time']} ({sched.get('source','?')})")
                        m["date"] = sched["date"]
                        m["time"] = sched["time"]
                        updated += 1
                        break
